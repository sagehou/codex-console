...
- `站点密码 (可选)` 对应 TempMail Worker 的 `x-custom-auth` 请求头；未启用时可以留空。
- `邮箱域名` 支持填写多个域名并使用逗号分隔，例如 `example.com, example.org`。
- 每次创建邮箱地址时会从已配置的域名列表中随机选择一个。
- 编辑已有 TempMail 服务时，站点密码输入框保持为空；留空表示保留当前密钥，输入新值表示替换，勾选“清空已保存的站点密码”会提交 `site_password: ""` 来删除已保存密钥。
